...
int main(int argc, char* argv[]) {
  ...
  auto f = fluent::fluent<ldb::PqxxClient>("key_value_server", client_address,
                                           &context, config)
               .stdin()
               .stdout();
  AddPrimalityApi(std::move(f))
      .RegisterRules([&](auto& stdin, auto& stdout, auto& req, auto& resp) {
        using namespace fluent::infix;

        auto in_to_req =
            req <=
            (stdin.Iterable() | ra::map([&](const std::tuple<std::string>& t) {
               return std::tuple<std::string, std::string, std::int64_t, int>(
