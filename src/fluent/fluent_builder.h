...
struct ValidateRules<common::TypeList<Rule<Collections, RuleTags, Ras>...>> {
  using all_collections =
      common::All<std::is_base_of<collections::Collection, Collections>...>;
  static_assert(common::StaticAssert<all_collections>::value, "");
  using all_rule_tags = common::All<std::is_base_of<RuleTag, RuleTags>...>;
  static_assert(common::StaticAssert<all_rule_tags>::value, "");
  using all_ras = common::All<std::is_base_of<ra::logical::LogicalRa, Ras>...>;
  static_assert(common::StaticAssert<all_ras>::value, "");
...
template <
    ...
    template <typename> class Hash = common::Hash,
    template <typename> class ToSql = lineagedb::ToSql,
    template <typename> class Pickler = common::CerealPickler,
...
class FluentBuilder<
  ...
  using WithCollection =
      FluentBuilder<common::TypeList<Collections..., Collection>,
                    BootstrapRules, UseTimestampWrapper, LineageDbClient, Hash,
                    ToSql, Pickler, Clock>;

  template <typename BootstrapRules_>
  using WithBootstrapRules =
      FluentBuilder<common::TypeList<Collections...>, BootstrapRules_,
                    UseTimestampWrapper, LineageDbClient, Hash, ToSql, Pickler,
                    Clock>;

  using WithTimestampWrapper =
      FluentBuilder<common::TypeList<Collections...>, BootstrapRules, true,
  ...
  static_assert(common::StaticAssert<all_collections>::value, "");
  static_assert(
      common::StaticAssert<detail::ValidateRules<BootstrapRules>>::value, "");
 ...
 public:
  ...
  WithCollection<collections::Table<Us...>> table(
      const std::string& name,
      std::array<std::string, sizeof...(Us)> column_names) && {
    LOG(INFO) << "Adding table " << name << "(" << common::Join(column_names)
              << ").";
    return AddCollection(std::make_unique<collections::Table<Us...>>(
        name, std::move(column_names)));
  }

  template <typename... Us>
  WithCollection<collections::Scratch<Us...>> scratch(
      const std::string& name,
      std::array<std::string, sizeof...(Us)> column_names) && {
    LOG(INFO) << "Adding scratch " << name << "(" << common::Join(column_names)
              << ").";
    return AddCollection(std::make_unique<collections::Scratch<Us...>>(
        name, std::move(column_names)));
  }

  template <typename... Us>
  WithCollection<collections::Channel<Pickler, Us...>> channel(
      const std::string& name,
      std::array<std::string, sizeof...(Us)> column_names) && {
    LOG(INFO) << "Adding channel " << name << "(" << common::Join(column_names)
              << ").";
    auto c = std::make_unique<collections::Channel<Pickler, Us...>>(
        id_, name, std::move(column_names), &network_state_->socket_cache);
    return AddCollection(std::move(c));
  }

  WithCollection<collections::Stdin> stdin() && {
    LOG(INFO) << "Adding stdin.";
    auto stdin_ptr = std::make_unique<collections::Stdin>();
    stdin_ = stdin_ptr.get();
    return AddCollection(std::move(stdin_ptr));
  }

  WithCollection<collections::Stdout> stdout() && {
    LOG(INFO) << "Adding stdout.";
    return AddCollection(std::make_unique<collections::Stdout>());
  }

  WithCollection<collections::Periodic<Clock>> periodic(
      const std::string& name,
      const typename collections::Periodic<Clock>::period& period) && {
    LOG(INFO) << "Adding Periodic named " << name << ".";
    auto p = std::make_unique<collections::Periodic<Clock>>(name, period);
 ...
 private:
  // Constructors //////////////////////////////////////////////////////////////
  static WARN_UNUSED common::StatusOr<FluentBuilder> Make(
  ...
  FluentBuilder(
    ...
    static_assert(
        common::TypeListLen<BootstrapRules>::value == 0,
        "This constructor must only be called on empty FluentBuilders.");
  }
  ...
  struct ValidateBootsrapRules {
    static_assert(common::TypeListLen<BootstrapRules>::value == 0,
                  "You are registering bootstrap rules with a FluentBuilder "
                  "that already has bootstrap rules registered with it. This "
                  "is disallowed.");
    static_assert(common::StaticAssert<common::IsTuple<RetTuple>>::value, "");
    static_assert(common::StaticAssert<detail::ValidateRules<RetTypes>>::value,
                  "");
  };
  ...
  template <
      ...
      typename RetTypes = typename common::TupleToTypeList<RetTuple>::type>
  ...
  template <
      bool UseTimestampWrapper_, typename F,
      typename RetTuple =
          typename std::result_of<F(TimestampWrapper&, Collections&...)>::type,
      typename RetTypes = typename common::TupleToTypeList<RetTuple>::type>
  typename std::enable_if<UseTimestampWrapper_,
                          WithBootstrapRules<RetTypes>>::type
  RegisterBootstrapRulesDispatchTimestamp(const F& f) {
  ...
  FluentBuilder<common::TypeList<Collections...>, RetTypes, UseTimestampWrapper,
                LineageDbClient, Hash, ToSql, Pickler, Clock>
  RegisterBootstrapRulesImplWithoutTimestampWrapper(
    ...
    static_assert(common::StaticAssert<correct_ret>::value, "");
    common::TupleIter(bootstrap_rules, [](const auto& rule) {
  ...
  WARN_UNUSED typename std::enable_if<
      ...
      common::StatusOr<FluentExecutor<common::TypeList<Collections...>,
                                      BootstrapRules, RetTypes, LineageDbClient,
                                      Hash, ToSql, Pickler, Clock>>>::type
  ...
  template <typename F, typename RetTuple, typename RetTypes, std::size_t... Is,
            typename Executor = FluentExecutor<
                common::TypeList<Collections...>, BootstrapRules, RetTypes,
                LineageDbClient, Hash, ToSql, Pickler, Clock>>
  common::StatusOr<Executor> RegisterRulesImplWithoutTimestampWrapper(
      const F& f, std::index_sequence<Is...>) {
    auto rules = f(*std::get<Is>(collections_)...);
    using correct_ret = std::is_same<decltype(rules), RetTuple>;
    static_assert(common::StaticAssert<correct_ret>::value, "");
    common::TupleIter(rules, [](const auto& rule) {
  ...
  template <typename F, typename RetTuple, typename RetTypes, std::size_t... Is,
            typename Executor = FluentExecutor<
                common::TypeList<Collections...>, BootstrapRules, RetTypes,
                LineageDbClient, Hash, ToSql, Pickler, Clock>>
  common::StatusOr<Executor> RegisterRulesImplWithTimestampWrapper(
      const F& f, std::index_sequence<Is...>) {
  ...
  template <template <template <typename> class Hash_,
            ...
            template <typename> class Hash_, template <typename> class ToSql_,
            template <typename> class Pickler_, typename Clock_>
  friend common::StatusOr<
      FluentBuilder<common::TypeList<>, common::TypeList<>, false,
                    LineageDbClient_, Hash_, ToSql_, Pickler_, Clock_>>
...
template <template <template <typename> class Hash,
          ...
          template <typename> class Pickler = common::CerealPickler,
          typename Clock = std::chrono::system_clock>
common::StatusOr<FluentBuilder<common::TypeList<>, common::TypeList<>, false,
                               LineageDbClient, Hash, ToSql, Pickler, Clock>>
fluent(const std::string& name, const std::string& address,
       zmq::context_t* context,
       const lineagedb::ConnectionConfig& connection_config) {
  return FluentBuilder<common::TypeList<>, common::TypeList<>, false,
                       LineageDbClient, Hash, ToSql, Pickler,
