...
struct ToDebugStringImpl<Project<Ra, Is...>> {
  std::string operator()(const Project<Ra, Is...>& project) {
    const std::string columns = common::Join(Is...);
...
struct ToDebugStringImpl<HashJoin<Left, LeftKeys<LeftKs...>,  //
  ...
  std::string operator()(
    ...
    const std::string left_keys = common::Join(LeftKs...);
    const std::string right = ToDebugString(hash_join.right);
    const std::string right_keys = common::Join(RightKs...);
...
struct ToDebugStringImpl<GroupBy<Ra, Keys<Ks...>, Aggregates...>> {
  std::string operator()(
    ...
    const std::string columns = common::Join(Ks...);
    const std::string aggregates = common::Join(Aggregates::ToDebugString()...);
...
std::string ToDebugString(const Ra& ra) {
  static_assert(
      common::StaticAssert<std::is_base_of<LogicalRa, RaDecayed>>::value, "");
